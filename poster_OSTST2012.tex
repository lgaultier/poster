...
% \begin{pcolumn}{0.32}
  \pbox{0.90\columnwidth}{28cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=1.0,framesep=1em}{
    ...
    \end{center}
    {\large
      \ding{226} Lack of resolution in time and space for current altimetric data $\rightarrow$ Observation of the submesoscale resolution ($\simeq$ 50~km) is not possible \\
      \ding{226} Submesoscale filaments can be observed using tracer sensors (SST or Ocean Color image, resolution of image$\simeq$ 1~km and a map a day)
      \begin{center}
...
\begin{center}
  \pbox{0.90\columnwidth}{25.0cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
     \begin{center}{\sc \bf \Large  NEW RESULTS: INVERSION ON A MODEL\\[5mm]} \end{center}
    {\large
\ding{226} An idealized coupled physico-biogeochemical model is set up to confirm the efficiency of the method. The model configuration is the following:\\
$\bullet$ Nemo dynmics in a channel is coupled with LOBSTER biogeochemical model. \\
...
\begin{center}
\includegraphics[width=1\linewidth]{./pict/sa_uv-R2BIGvel_0324_pert.eps}%{0.99}
\\
...
\begin{minipage}{0.21\textwidth}
\begin{center}
\includegraphics[width=1\linewidth]{./pict/sa_uv-R2BIGvel_0324_0070.eps}%{0.99}
\\
{\small \bf Corrected velocity}
\end{center}
\end{minipage}
\begin{minipage}{0.21\textwidth}
\begin{center}
\includegraphics[width=1\linewidth]{./pict/sa_uv-R2BIGvel_0324_F.eps}%{0.99}
\\
