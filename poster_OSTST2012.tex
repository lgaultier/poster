...
% \begin{pcolumn}{0.32}
  \pbox{0.92\columnwidth}{30cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=1.0,framesep=1em}{
    ...
    \end{center}
    {\large
      \ding{52} Lack of resolution in time and space for altimetrice Data $\rightarrow$ Observation of the submesoscale resolution ($\simeq$ 50~km) not possible
      \ding{52} Submesoscale filaments can be observed using tracer sensors (SST or Ocean Color image, resolution of image as low as 200 m and a map a day)
      \begin{center}
      ...
      \end{center}
      \ding{52} Some studies brought to light the connection between mesoscale velocities and tracer patterns \citep{Dovidio2004, Lehahn2007}, but correcting mesoscale velocities using tracer images has never been done before.
%%%   \begin{center}
      \begin{tikzpicture}
...
\begin{center}
  \pbox{0.90\columnwidth}{55cm}{linewidth=2mm, framearc=0.1, linecolor=mycyan, fillstyle=gradient, gradangle=0, gradbegin=white, gradend=white, gradmidpoint=1.0, framesep=1em}{
    \begin{center}
      {\sc \bf \Large DATA IMAGE ASSILATION\\[5mm]}
    \end{center}
    \vspace*{0.2cm}
    {\large
...
\fi
    ...
    \node[pname,text width=10cm] (TRA) at (20.5,6.48) {Biogeochemical tracer};
    \node[pname,text width=10cm] (DYN) at (32.5,6.48) {Dynamic};
...
\begin{center}
  \pbox{0.85\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=mycyan,gradend=mycyan, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{Goal: Find the optimal velocity $\vec{u}$ that minimizes the cost function J.} 
\end{center}
  }
    \begin{center}
    ...
    \vspace*{0.2cm}
    {\large
See \cite{Gaultier2012} for details about the method. 
  
  \ding{52} Compute a subspace of error to perturb the velocity \\
  \ding{52} Explore the subspace of error \\
  \ding{52} Use a simulated annealing algorithm to decrease the cost function: \\
Initialisation: \\
Background velocity $\vec{u_b}$: Velocity map derived from AVISO observation\\
Cost function: Distance between AVISO velocity and tracer observation\\
Loop: \\
Perturb the velocity: $\vec{u_p}=\vec{u} + \delta \vec{u}$ \\
Compute the cost function $J(\vec{u_p})$ and the variation of the cost function $\deltaJ=J(\vec{u})-J(\vec{u_p})$ \\
  \ding{226}  If $\deltaJ \leqslant 0$ $\rigtharrow$ Accept perturbation ($\vec{u}=\vec{u_p}$) \\
  \ding{226} If $\delta J\seqslant 0$ $\rigtharrow$ Accept perturbation with probability $e^{-\frac{\deltaJ}{T}}$ \\
Exit when a minimum of the cost function is reached. \\

  \ding{52} Use a Gibbs Sampler to assess possible solution around the minimum of the cost function\\ 
  ...
  \pbox{0.90\columnwidth}{21.5cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
    ...
    {\large
     \begin{center}
      ...
      \end{center}
      \ding{52} The corrected velocity is more consistent with the tracer field than the observed one, and the incertitude on this result is small. 
       However, the method still needs to be improved, since some areas of the velocity field are not accurately corrected. \\    
      \ding{52} We plan on using a high resolution model to refine the method. 
      ...
      \ding{52} This study opens the way for the use of very high resolution altimeter data (in the context of SWOT and SARAL projects). The strategy proposed in here enables us to handle huge amount of data in models. 
