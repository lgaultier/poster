...
% \begin{pcolumn}{0.32}
  \pbox{0.92\columnwidth}{30cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=1.0,framesep=1em}{
    ...
    \end{center}
    {\large
      \ding{226} Altimetric satellites are currently limited to the observation of the dynamic at a mesoscale resolution ($\simeq$ 50~km) and can provide at best a map a week. whereas tracer sensors have a resolution as low as 200 m and can provide a map a day. For example, Satellite observations of Sea Surface Temperature (SST) or Ocean Color show sub-mesoscale dynamics.
      \begin{center}
...
%  \draw[->, thick,double distance=4pt , >=stealth] (pTRA)--(10,0);
      ...
      \end{tikzpicture}\\ 
      %\ding{226} 
      The tracer and the velocity are physically different, therefore we use Finite-Size Lyapunov Exponents (FSLE) as a go-between variable to enable the velocity field and the tracer image to discuss together
...
%   \begin{minipage}{0.49\linewidth} 
      \begin{pcolumn}{0.49}
        \pbox{0.9\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{
           ...
           \small{SSH (in cm) in the Western Mediterranean Sea on July 9$^{th}$, 2003. The test case is located in the purple rectangle}
        } 
      \end{pcolumn}
%   \vline
      \begin{pcolumn}{0.49}
        \ding{226} Domain: [38.2\DegN ; 40\DegN] $\times$ [4.8\DegE ; 8\DegE]. \\ 
        It has been chosen because of: 
...
%      \hline
        High resolution SST images from MODIS sensor at 1~km resolution: & 
        Maps of velocity derived from AVISO altimetric observations at 1/8\Deg resolution: \\ 
        \hline
...
%        \hline
    ...
    \end{tikzpicture}
    \end{center}
A neat gyre can be observed in both pictures.
...
%\begin{minipage}{0.3\textwidth}
  ...
  \pbox{0.60\columnwidth}{21.5cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
    ...
    {\large
      ...
      \end{center}
      \vspace{1em}
      \ding{226} In accordance with the implemented method, velocity vectors are corrected so as to follow tracer frontal structures. For instance, the North East of the gyre in the corrected velocity (blue rectangle) is much more consistent with the SST frontal structure than the North East of the gyre in the observed AVISO velocity. \\  
