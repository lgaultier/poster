...
%%% Logo
  ...
  \begin{center}
    \includegraphics[width=0.80\textwidth,angle=0]{./pict/logo_cnrs.eps}
    \includegraphics[width=0.80\textwidth,angle=0]{./pict/logo_cnes2.eps}
  \end{center}
  \end{minipage}
%%% Titre
...
%On the inversion of sub-mesoscale information\\ 
 %                  \vspace{0.5em}
  %                 to correct mesoscale velocity}\\[10mm]
...
%%%Logo
  ...
  \begin{center}
    \includegraphics[width=0.74\textwidth,angle=0]{./pict/logo_meom.eps}
    \includegraphics[width=0.74\textwidth,angle=0]{./pict/logo_legi.eps}
  \end{center}
  \end{minipage} 
  }  %pbox
  \end{center}
...
% \begin{pcolumn}{0.32}
  \pbox{0.92\columnwidth}{30cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=1.0,framesep=1em}{
    ...
    \end{center}
    {\large
      \ding{226} It is difficult to access the sub-mesoscale using altimetric satellites since they provide data along distant tracks whereas tracer sensors provide images at a resolution as low as 200 m.
%Altimetric satellites are currently limited to the observation of the dynamics at a mesoscale resolution ($\simeq$ 20~km) and can provide at best a map a week. whereas tracer sensors have a resolution as low as 200 m and can provide a map a day.
       For example, Satellite observations of Sea Surface Temperature (SST) or Ocean Color show sub-mesoscale dynamics.
      \begin{center}
      ...
      \end{center}
      \ding{226} Some studies brought to light the connection between mesoscale velocities and tracer patterns \citep{Dovidio2004, Lehahn2007}, but correcting mesoscale velocities using tracer images has never been done so far.
%. This poster presents the strategy made up to correct mesoscale altimetric velocity using a tracer image.
%   \begin{center}
      \begin{tikzpicture}
%    \node[color=blue, text width=10cm, text centered] (UV) at (16,6.8) {Mesoscale field};
        ...
        \node[color=black, text width=20cm, text centered] (cUV) at (19,-6.) {\small{AVISO Velocity map, Tasmania region, December 22, 2004}};
...
%  \draw[->, thick,double distance=4pt , >=stealth] (pTRA)--(10,0);
      ...
      \end{tikzpicture}\\ 
      %\ding{226} 
      The tracer and the velocity designate different ocean variables that are not simply linked. The Finite-Size Lyapunov Exponents (FSLE) is used as a go-between variable to enable the velocity field and the tracer image to speak together.
...
%   \begin{minipage}{0.49\linewidth} 
      \begin{pcolumn}{0.49}
        \pbox{0.9\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{
           \myfig[0]{./pict/aviso_h_019904_bmed.eps}{0.8}
           \small{SSH (in cm) in the Western Mediterranean Sea, first week of July, 2004. The test case is located in the purple rectangle}
        } 
      \end{pcolumn}
%   \vline
      \begin{pcolumn}{0.49}
        \ding{226} Domain: [38.2\DegN ; 40\DegN] $\times$ [4.8\DegE ; 8\DegE] \\ 
        chosen because of: 

 
%        \indent \ding{51} the good quality of the data sets \\ 
%        \hspace{1cm} \ding{51} the strong filament signature \\ 
        \hspace{1cm} \ding{51} the good quality of the data set


        \hspace{1cm} \ding{51} the strong filament signature


        \ding{226} Study date: July 2$^{nd}$, 2004 \\  
        \ding{226} Time Range to estimate the variability of the velocity: 1998-2009, 595 velocity maps
...
%    \vfill
%\vspace{0.8cm}
    \ding{226} Two satellite observation data sets are considered: \\ 
    \begin{center}
    \begin{tikzpicture}
      \node (table) [inner sep=0pt] {
      \begin{tabular}{m{19cm}|m{19cm}}
%      \hline
        \textcolor{mycyan}{\bf Tracer used as a dynamic information}: high resolution SST images from MODIS sensor at 1~km resolution: & 
        \textcolor{mycyan}{ \bf Background velocity to be corrected}: map of velocity derived from AVISO altimetric observations at 1/8\Deg resolution: \\ 
        \hline
        \myfig[0]{./pict/A2004184123500_L2_LAC_SST.eps}{0.6} 
        \small{SST image from MODIS sensor (in \Deg C) on July 2$^{nd}$, 2004} &
        \myfig[0]{./pict/aviso_velmap19904_med.eps}{0.6} 
        \small{Geostrophic Velocity (in m.s$^{-1}$) over the SSH (in cm) on the first week of July, 2004 from AVISO mapped products} \\ 
%       \hline
...
%        \hline
    ...
    \end{tikzpicture}
    \end{center}
A neat eddy can be observed in both pictures.
The FSLE are computed at 1/48\Deg resolution. The SST image is filtered and coarsen at the same resolution (1/48\Deg).
...
\pbox{0.96\columnwidth}{34cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
  ...
  {\large
   ...
   %   \vspace{1cm}
      ...
      \ding{226} FSLE measure stirring in a fluid, It is a \textcolor{mycyan}{ \bf connection between
sub-mesoscale dynamics and tracer stirring.}\\ 
...
%      Technically, four particles are initially at a distance $\delta_0$ from each other, we compute the time $\tau$ for these particles to reach the distance $\delta_f$ from the other four particles. 
      ...
      \pbox{0.9\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{
        \myfig{./pict/fsle_48_stat_reg_19904_med.eps}{0.9}
        \small{FSLE (in day$^{-1}$) derived from the geostrophic AVISO velocity first week of July, 2004} 
      }
      \end{minipage}
      \begin{minipage}{0.49\textwidth}
        \textcolor{mycyan}{ \bf There are similar patterns between the maximum lines of Lyapunov exponents and SST frontal structure}
(that is to say the norm of the gradient). 
      ...
      \vfill
      \ding{226} \textcolor{mycyan}{ \bf We want to find the FSLE field as close as possible to the normalized tracer gradient.} 
      Therefore, the FSLE field and the normalized SST gradients are binarized in order to compare those two physically different variables. 
        $$\hat{\lambda}=
   \left \{
   \begin{array}{r r r}
   ...
   \vspace{1cm}
    ...
    \begin{pcolumn}{0.36}
      ...
      \ding{226} \textcolor{mycyan}{ \bf The cost function $J$ measures the distance} between the binarized normalized gradient of SST $\hat{\lambda}_o$ and the binarized FSLE $\hat{\lambda}({\bf u})$ (as a proxy of the velocity ${\bf u}$). 
      $$J({\bf u}) = \mu \; || \hat{\lambda}_o - \hat{\lambda}({\bf u}) ||^2 + BK({\bf u})$$
       $BK({\bf u})$ is the background term. Assuming that the error on the altimetric observation is small, the solution velocity is chosen to be close to the background AVISO velocity field.
      ...
      \pbox{0.9\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{
        ...
        \begin{minipage}{0.47\textwidth}
          ...
          \small{Binarization of the FSLE derived from the AVISO velocity,first week of July 2004}
        \end{minipage}
      }
      \end{center}
      \vspace{0.5em}

      \ding{226} \textcolor{mycyan}{ \bf A velocity sub-space is created using Principal Component Analysis with an ensemble of velocities.} \\ 
      Let S = $|u^{(1)}, u^{(2)}, ..., u^{(r)}|$ be the first r EOFs. 
      The velocity errors are considered with zero mean and covariance of the ensemble of velocities $S$: $\delta u \sim \mathcal{N}(0,SS^T)$ 
...
%Find the velocity that minimizes the cost function among the perturbed velocities
    ...
    \begin{pcolumn}{0.32}
      ...
      Nevertheless, \textcolor{mycyan}{ \bf the cost function $J(u)$ is quite irregular} and many local minima can be found. 
      To avoid being stuck in one of them, \textcolor{mycyan}{ \bf the minimization of the cost function is performed using a Simulated Annealing.} \\
%      At each step, a perturbation is computed and the resulting cost function is calculated, if it is lower than the previous one, we adopt the perturbation, otherwise, we reject the perturbation with a certain probability. \\ 
      ...
      \end{minipage}
      \begin{minipage}{0.49\textwidth}
        At each step, a perturbation is computed and the resulting cost function is calculated, if it is lower than the previous one, the perturbation is adopted, otherwise, the perturbation is rejected with a certain probability. \\
      \end{minipage}
      ...
      To do so, a Gibbs Sampler is used. 
      In other words, all the perturbations which are potential solutions to the problem are assessed. 
      An optimal solution is given by the velocity that has the lowest cost function among all the potential solutions. 
%      A potential solution that has the lowest cost function gives us an optimal solution, 
      The variance of all the potential solutions indicates how reliable an optimal solution is. 
...
%\begin{minipage}{0.3\textwidth}
  \begin{center}
  \pbox{0.60\columnwidth}{21.5cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
    \begin{center}
    ...
    {\large
      ...
      \pbox{0.9\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{
        \begin{minipage}{0.32\textwidth}
          \myfig{./pict/aviso_19904_med_circle.eps}{0.8}
      ...
      \end{center}
      \vspace{1em}
      \ding{226} In accordance with the implemented method, \textcolor{mycyan}{ \bf velocity vectors are corrected so as to follow tracer frontal structures.}
...
%The corrected velocity is a better match to the tracer than the Aviso velocity. \\ 
      %the new velocity is more consistent with the tracer field than the old one.
      \ding{226} Nevertheless, there are some limits to this method. 
...
%Some areas are not corrected or miscorrected, this is due to the remaining noise in the binarized tracer image. \\ 
%      \ding{226} Several improvements are considered to improve the process of extracting dynamic information from the tracer observation. 
%       Some techniques to erase small structures due to the noise or to elongate the filaments structure. \\ 
       \ding{226} The variance of all the possible velocities is rather small compared to the value of the velocity and the correction. 
  ...
  \pbox{0.32\columnwidth}{21.5cm}{linewidth=2mm,framearc=0.1,linecolor=mycyan,fillstyle=gradient,gradangle=0,gradbegin=white,gradend=white,gradmidpoint=1.0,framesep=1em}{
    \begin{center}{\sc \bf \Large  CONCLUSION\\[5mm]} \end{center}
    {\large
     \begin{center}
      \pbox{0.90\columnwidth}{}{linewidth=0.5mm,framearc=0.2,linecolor=black, fillstyle=gradient,gradangle=0,gradbegin=mycyan,gradend=mycyan, gradmidpoint=0.0,framesep=3mm, shadowsize=0pt}{ \bf We succeeded in correcting an altimetric mesoscale velocity field using a sub-mesoscale tracer observation from space }
      \end{center}
      \ding{226} \textcolor{mycyan}{ \bf The corrected velocity is more consistent with the tracer field than the observed one}, and the uncertainty on this result is small. 
       The method still needs to be improved, since some areas of the velocity field do not seem consistently corrected. \\    
%      New velocity more consistent with the tracer field than the old one. \\ 
 %     Still some wrong corrections \\ 
      \ding{226} We plan to use a \textcolor{mycyan}{ \bf high resolution model to refine the method}. 
...
%      Next step: use a model to improve the method knowing the truth \\ 
